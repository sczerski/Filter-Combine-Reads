    ...
    with open(all_filtered_reads_w_id, "r") as f:
        ...
        with open("all_reads_info.tsv",  'w') as ari_tsv:
            ...
            for i in pre_reads_info_ncbi:
                ...
                extra_info = post_reads_info_ncbi[5].split(sep=" ")
                genus_species = extra_info[1] + "_" + extra_info[2]
                gi_num = post_reads_info_ncbi[0] + "_" + post_reads_info_ncbi[1]
                no_quotes = ""
                i=3
                ...
                post_reads_info_ncbi.pop(5)
                post_reads_info_ncbi.pop(2)
                post_reads_info_ncbi.pop(0)
                post_reads_info_ncbi.pop(0)


                #appending new consolidated elements
                post_reads_info_ncbi.append(gi_num)
                ...
                swap_element_positions(post_reads_info_ncbi, 0, 1)
                swap_element_positions(post_reads_info_ncbi, 2, 3)
                swap_element_positions(post_reads_info_ncbi, 0, 3)
...
if __name__ == '__main__':
    #Get Input
    otu_fasta_file = input("Input path to OTU fasta file (eg. OTU_X_headers.tsv.fasta): ") #"OTU_45_headers.tsv.fasta"
    ncbi_records = input("Input path to ncbi fasta file (eg. X_sequence.fasta): ") #"anaerococcus_sequence.fasta"
    ...
    new_records = filter_single_reads(otu_fasta_file)

    #Combine filtered reads with the reads downloaded from ncbi
    all_filtered_records = combine_pacbio_w_ncbi(new_records, ncbi_records)
